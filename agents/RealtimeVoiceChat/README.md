...
2.  **Start the services (App & Ollama):**
    *(Runs containers in the background. GPU access is configured in `docker-compose.yml`.)*
    ...
    > ⚙️ **Model bootstrapping** – the Ollama service is configured to
    automatically pull a default LLM model on first startup (the
    default is `llama2:latest`).  You do **not** need to manually run
    `ollama pull` yourself; the container will download the model into
    the `ollama_data` volume and then start serving it.  If you prefer a
    different or smaller model, set `OLLAMA_DEFAULT_MODEL` in the
    `docker-compose.yml` or your environment before running the command.

    Monitor the download progress with:

    ```sh
    docker logs -f realtime-voice-chat-ollama | grep --line-buffered downloading
    ```

    Once the pull completes and `ollama ps` returns a model entry, the
    backend will be able to prewarm and respond to your voice input.
