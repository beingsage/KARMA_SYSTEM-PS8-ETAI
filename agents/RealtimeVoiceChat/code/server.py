...
USE_SSL = False
TTS_START_ENGINE = "kokoro"  # Using Kokoro (lightweight, CPU-friendly)
TTS_ORPHEUS_MODEL = "Orpheus_3B-1BaseGGUF/mOrpheus_3B-1Base_Q4_K_M.gguf"
TTS_ORPHEUS_MODEL = "orpheus-3b-0.1-ft-Q8_0-GGUF/orpheus-3b-0.1-ft-q8_0.gguf"

LLM_START_PROVIDER = "ollama"  # Use local Ollama server if available
# Default LLM model to use with Ollama (can be a local model name)
# When using the local Ollama container, this value must correspond to a model
# that has been pulled into the container. The default above points at a large
# HF-hosted GGUF model which we haven't downloaded yet, so pick a lightweight
# built‑in identifier such as "llama2" for a quick demo. You can change it back
# to any other name once the model is available inside Ollama (e.g.
# "hf.co/your/model:alias").
LLM_START_MODEL = "llama2"
...
static_dir = os_path.path.join(os_path.path.dirname(__file__), "static")
...
async def favicon():
    ...
    """
    # Use the configured static directory so this works regardless of cwd
    return FileResponse(os_path.path.join(static_dir, "favicon.ico"))
...
async def get_index() -> HTMLResponse:
    ...
    """
    index_path = os_path.path.join(static_dir, "index.html")
