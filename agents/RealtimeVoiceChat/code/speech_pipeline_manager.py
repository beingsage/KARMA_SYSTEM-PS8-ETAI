...
try:
    from app.voice_runtime import build_copilot_response_provider
except Exception:
    build_copilot_response_provider = None
...
class SpeechPipelineManager:
    ...
    def __init__(
        ...
        # Ensure a numeric default so formatting/logging doesn't fail when
        # the LLM backend is not available (no API key or local server).
        ...
        self.copilot_response_provider = None
        if build_copilot_response_provider is not None:
            try:
                self.copilot_response_provider = build_copilot_response_provider()
            except Exception as exc:
                logger.warning(f"🗣️⚠️ Could not initialize Copilot voice bridge: {exc}")

        # Protect against None values from audio/tts measurement
        audio_tts_time = getattr(self.audio, 'tts_inference_time', None)
        if audio_tts_time is None:
            audio_tts_time = 0.0
        self.full_output_pipeline_latency = self.llm_inference_time + audio_tts_time
        logger.info(f"🗣️⏱️ Full output pipeline latency: {self.full_output_pipeline_latency:.2f}ms (LLM: {self.llm_inference_time:.2f}ms, TTS: {audio_tts_time:.2f}ms)")
    ...
    def process_prepare_generation(self, txt: str):
        ...
        try:
            ...
            if self.copilot_response_provider:
                self.running_generation.llm_generator = self._build_copilot_generator(txt)
                logger.info(f"🗣️🧠✔️ [Gen {new_gen_id}] Copilot voice bridge created. Setting generator ready event.")
            else:
    ...
    def _build_copilot_generator(self, txt: str):
        def generator():
            answer = self.copilot_response_provider(txt, job_id=None, use_web=True)
            yield answer
