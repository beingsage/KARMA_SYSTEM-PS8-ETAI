...
# Allow overriding via environment, default to our commonly used local port (11435) after earlier conflicts
OLLAMA_BASE_URL = os.getenv("OLLAMA_BASE_URL", "http://127.0.0.1:11435")
if "OLLAMA_BASE_URL" in os.environ:
    logger.info(f"🤖⚙️ Using OLLAMA_BASE_URL from environment: {OLLAMA_BASE_URL}")
else:
    logger.info(f"🤖⚙️ Using default OLLAMA_BASE_URL: {OLLAMA_BASE_URL}")
...
class LLM:
    ...
    def __init__(
        ...
        self.model = model
        # Normalize model name for Ollama: append ':latest' if no tag provided
        if self.backend == "ollama" and isinstance(self.model, str) and ":" not in self.model:
            old_model = self.model
            self.model = f"{self.model}:latest"
            logger.info(f"🤖⚙️ Normalized Ollama model name from '{old_model}' to '{self.model}'")
